...
A tkinter-based GUI that wraps all of the scripts above into a single window with one tab per script. Each tab has a folder picker and script-specific options (e.g. format and dry-run for `rename-mp3s.py`, source/target pickers and sort options for `fuzzy-find-files.py`). Output streams live into the window as the script runs.
